...
both_baseline = add_models(baseline_models_d, baseline_models_p)
...
def get_new_car_weighting(latest_year):
    baseline_new_cars = latest_year.get_counts(age=0)
...
def generate_next_year(year_models, full_sales, rounded_car_count, fuel):
    ...
    weightings = get_new_car_weighting(latest_year)
...
def final_step(year, cc, fuel):
    write_year_model_to_csv(cc, year, fuel)
